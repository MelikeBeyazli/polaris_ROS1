...
class LineFollower:
    ...
    def process_frame(self, img):
        ...
        if M['m00'] > 0:
            ...
            if left_sum > 0.4 * np.sum(maske) and right_sum > 0.4 * np.sum(maske):
                # İki tarafta da çizgi varsa yol ayrımı var
                print("Yol ayrımı tespit edildi")
                # Burada karar verme mantığını ekleyebilirsiniz
            else:
                print(f"Sapma: {sapma}")
            return sapma
        else:
